...
const initialUsers = [
    { name: 'Melaku', username: 'superadmin', password: 'superadmin', role: 'superadmin', active: true },
    { name: 'Abebe', username: 'admin', password: 'admin', role: 'admin', active: true },
    { name: 'Demeke', username: 'teacher', password: 'teacher', role: 'teacher', active: true },
    { name: 'Abebech', username: 'librarian', password: 'librarian', role: 'librarian', active: true },
    { name: 'Kebede', username: 'parent', password: 'parent', role: 'parent', active: true },
    { name: 'Maroof', username: 'student', password: 'student', role: 'student', active: true }
];
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const userMess = document.getElementById('user-mess');
    ...
    if (loginForm) {
        loginForm.addEventListener('submit', function(event) {
            ...
            if (user && user.password === password) {
                // Check user status
                if (user.active) {
                    const userRole = user.role; // Assuming user.role exists
                    alert(`Login successful! Redirecting to ${userRole}'s dashboard...`);
                    window.location.href = userRole + '.html'; // Redirect to the corresponding dashboard
                } else {
                    document.getElementById('error-message').textContent = 'Your account is inactive. Please contact the administrator.';
                }
            } else {
    ...
    userForm.addEventListener('submit', function(event) {
        ...
        if (users.some(user => user.username === username)) {
            userMessage.textContent = 'Username already exists. Please choose a different one.';
            userMessage.classList.add('error-message'); // Optional: Add a class for styling
            return; // Exit the function if username exists
        }

        // Add new user if username is unique
        users.push({ name: username, username, password, role, active: true });
        localStorage.setItem('users', JSON.stringify(users));
        displayUsers(); // Refresh the user list
        userForm.reset(); // Reset the form
        userMess.textContent = 'User added successfully!'; // Success message
    });
});

document.getElementById('show-add-user').addEventListener('click', function() {
    // Logic to display the add user form
...
function loadMessages() {
  ...
  if (messages.length === 0) {
    messagesDiv.innerHTML = '<div><p style="color: red;">No messages yet. Start the conversation!</p></div>';
  } else {
    messages.forEach(msg => {
        messagesDiv.innerHTML += `<div><p>${msg}</p></div>`;
    });
  }
}
...
function deleteMessages() {
  if (confirm("Are you sure you want to delete the chat history?")) {
      localStorage.removeItem('messages'); // Clear chat history
